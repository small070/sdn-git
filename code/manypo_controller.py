...
class good_controller(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, priority, match, actions):
        ...
        mod = parser.OFPFlowMod(datapath=datapath, priority=priority, command=ofp.OFPFC_ADD,
                                match=match, instructions=inst, hard_timeout=50)# ofp.OFPP_FLOOD
            ...
            for i in range(len(path) - 1):
                ...
                if datapath.id == path[-1]:
                    ...
                    if i == 0:
                        ...
                        #flooding
                        match2 = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP, ipv4_src=pkt_ipv4.dst,
                                                ipv4_dst=pkt_ipv4.src, in_port=request_port)
                        out_port = ofproto.OFPP_FLOOD
                        actions2 = [parser.OFPActionOutput(out_port)]
                        self.add_flow(test_datapath, 1, match2, actions2)
                        ...
                        #flooding
                        match2 = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP, ipv4_src=pkt_ipv4.src,
                                                ipv4_dst=pkt_ipv4.dst, in_port=port)
                        out_port = ofproto.OFPP_FLOOD
                        actions2 = [parser.OFPActionOutput(out_port)]
                        self.add_flow(test_datapath1, 1, match2, actions2)
...
class MLDetection(good_controller):
    ...
    def _monitor(self):
        while True:
            ...
            self.dataset.to_csv('my_manypo_test.csv', mode='a', header=False)
            ...
            self.dataset2.to_csv('ref_manypo_test.csv', mode='a', header=False)
    ...
    def aggregate_stats_reply_handler(self, ev):
        ...
        print('SPI: body.flow_count  ', body.flow_count)
        print('SPI: degree.request_sid  ', degree.request_sid)
        print('SPI: ev.msg.datapath.id  ', ev.msg.datapath.id)
        print('SPI: degree[degree.request_sid == int(ev.msg.datapath.id)].swdegree  ', degree[degree.request_sid == int(ev.msg.datapath.id)].swdegree)
        print('SPI: degree[degree.request_sid == int(ev.msg.datapath.id)].swdegree.values  ', degree[degree.request_sid == int(ev.msg.datapath.id)].swdegree.values)
