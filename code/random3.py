...
class CustomTopo(Topo):
    ...
    def build(self):
        ...
        for i in range(num_switches - 1):
            ...
            cur_link = [i + 1, i + 2]
            if cur_link[0] not in list(plinks.keys()):
                plinks[cur_link[0]] = {}
            if (cur_link[0] not in list(port_counter.keys())):
                port_counter[cur_link[0]] = 1
            plinks[cur_link[0]][cur_link[1]] = port_counter[cur_link[0]]
            port_counter[cur_link[0]] += 1

            if cur_link[1] not in list(plinks.keys()):
                plinks[cur_link[1]] = {}
            if (cur_link[1] not in list(port_counter.keys())):
